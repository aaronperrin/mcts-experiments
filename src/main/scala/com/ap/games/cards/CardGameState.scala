...
object CardGameState {
  ...
  def initialGameState(context: GameContext, monitor: Monitor) = CardGameState(
    context,
    InitialLibrary,
    None,
    Hero(24, 24, 3, 3, 0, 5),
    maybeMonitor = Some(monitor)
  )
}

case class CardGameState(context: GameContext, library: List[Card], maybeEncounter: Option[EncounterState], hero: Hero, prevEncounters: List[EncounterState] = Nil, maybeMonitor: Option[Monitor] = None, isEnded: Boolean = false) {
  ...
  def nextState(action: CardGameAction): CardGameState = {
    action match {
      ...
      case _ =>
        maybeEncounter
          .map(encounter => copy(maybeEncounter = Some(encounter.nextState(action))))
